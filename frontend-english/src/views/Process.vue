    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" title="刷新Graph">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">Graph数据Loading...</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">Knowledge Graph Construction中</p>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        ...
        <div class="process-content">
          ...
          <!-- 阶段2: Knowledge Graph Construction -->
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">接口说明</div>
                <div class="detail-content">
                  Based on the generated ontology，将文档分块后调用 Zep API 构建知识Graph，Extract entities and relations
                </div>
              </div>
...
const currentPhase = ref(-1) // -1: 上传中, 0: Ontology Generation中, 1: Knowledge Graph Construction, 2: 完成
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Knowledge Graph Construction中'
...
const handleNewProject = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      // 自动开始Knowledge Graph Construction
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: '正在启动Knowledge Graph Construction...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Knowledge Graph Construction任务已启动...'
      ...
      error.value = response.error || '启动Knowledge Graph Construction失败'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = '启动Knowledge Graph Construction失败: ' + (err.message || '未知错误')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Knowledge Graph Construction完成，正在加载完整数据...')
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Knowledge Graph Construction失败: ' + (task.error || '未知错误')
        buildProgress.value = null
      }
    }
  } catch (err) {
