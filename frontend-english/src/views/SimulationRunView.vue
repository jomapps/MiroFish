...
const minutesPerRound = ref(30) // 默认每Rounds30min
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Time Configuration: 每Rounds ${minutesPerRound.value} min`)
        }
      } catch (configErr) {
        addLog(`获取Time Configuration失败，使用默认值: ${minutesPerRound.value}min/Rounds`)
      }
...
onMounted(() => {
  ...
  // 记录 maxRounds Config（值已在初始化时从 query 参数获取）
  if (maxRounds.value) {
