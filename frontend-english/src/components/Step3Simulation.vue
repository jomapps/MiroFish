...
// 格式化模拟流逝时间（根据Roundstimes和每Roundsmin数计算）
...
// 追踪各平台的上一timesRoundstimes，用于检测变化并输出日志
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ 启动Report Generation失败: ${res.error || '未知错误'}`)
...
onMounted(() => {
  addLog('Step3 Simulation Run Init')
  if (props.simulationId) {
    doStartSimulation()
  }
})
