    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Creating a new simulation instance and fetching templates.
          </p>
      ...
      <!-- Step 02: Generating Agent Personas -->
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">02</span>
            <span class="step-title">Generating Agent Personas</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Calling tools to extract entities and relations from the knowledge graph，initializing simulation agents，and assigning unique behaviors and memories.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM 根据Simulation Requirement与Reality Seed，智能设置世界时间流速、Recommendation Algorithms、Agent Active Hours、Post Frequency、Event Triggers, etc.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Algo Config</span>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Generating initial events and topics based on narrative direction，to initialize the simulation state
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">The environment is ready. You can now start the simulation.</p>
          
          <!-- 模拟Rounds数Config - 只有在Config生成完成且Rounds数计算出来后才显示 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">模拟Rounds数设定</span>
                <span class="section-desc">MiroFish 自动规划推演现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours，每Rounds代表现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> min时间流逝</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">若首times运行，强烈建议切换至‘自定义模式’减少模拟Rounds数，以便快速预览效果并降低报错风险 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="action-group dual">
            <button 
              ...
              @click="$emit('go-back')"
            >
              ← 返回Knowledge Graph Construction
            </button>
            <button 
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">Reality Seed关联话题</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">基于Reality Seed形成的记忆</span>
              </div>
              <div class="dimension-card">
...
// State
const phase = ref(0) // 0: 初始化, 1: 生成人设, 2: 生成Config, 3: 完成
...
// 日志去重：记录上一times输出的关键信息
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// 模拟Rounds数Config
const useCustomRounds = ref(false) // 默认使用自动ConfigRounds数
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人设' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === '生成模拟Config' || newStage === 'generating_config') {
    phase.value = 2
    // 进入Config生成阶段，开始Rounds询Config
    if (!configTimer) {
      addLog('开始Generate Dual Platform Config...')
      startConfigPolling()
    }
  } else if (newStage === '准备模拟脚本' || newStage === 'copying_scripts') {
    phase.value = 2 // 仍属于Config阶段
  }
})

// 从Config中计算自动生成的Rounds数（不使用硬编码默认值）
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`Start Simulation，使用自动ConfigRounds数: ${autoGeneratedRounds.value} Rounds`)
  }
...
const startPrepareSimulation = async () => {
  ...
  addLog('正在Preparing Environment...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 立即设置Total Expected Agents（从prepare接口返回值获取）
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog('正在生成Agent人设Config...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('正在调用LLM生成模拟Config参数...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time Configuration: 每Rounds${tc.minutes_per_round}min, 共${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}Rounds`)
        }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('ConfigGenerating，开始Rounds询Waiting...')
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(`加载Config失败: ${err.message}`)
    emit('update-status', 'error')
  }
}
